...
export default function SharePage() {
  ...
  // Build scores from URL only (server-safe):
  const { labels, serverValues } = useMemo(() => {
    const urlScores = decodeScores(encoded);
    const labels = DEFAULT_LABELS.slice();
    const serverValues = labels.map((l) => urlScores[l] ?? 0);
    return { labels, serverValues };
  }, [encoded]);

  // Client-side effect to handle localStorage fallback
  useEffect(() => {
    setIsClient(true);
    
    // If URL has valid data, use it
    if (encoded && !serverValues.every(v => v === 0)) {
      setClientValues(serverValues);
      return;
    }
    ...
    try {
      ...
      const localValues = labels.map((l) => local[l] ?? 0);
      setClientValues(localValues);
    } catch {
      setClientValues(serverValues);
    }
  }, [encoded, serverValues, labels]);

  // Use server values initially, then client values after hydration
  const values = isClient ? clientValues : serverValues;
